...
#   FEDOR: addicional overlap filtering
    python3 $wd/overlap_filter.py $wd/_nhmmer-t0-$bn.bed > $wd/AS-SF-vs-$bn.bed

#   FEDOR: AS-strand annotation. "+" is blue, "-" is red
    awk -F $'\t' 'BEGIN {OFS = FS} {if ($6=="+") {$9="0,0,255"}; if ($6=="-") {$9="255,0,0"} print $0}' $wd/AS-SF-vs-$bn.bed > $wd/AS-strand-vs-$bn.bed
